...
roots = []
# roots.append("D:/Documents/AirSim/Scene1_Data/daytime/")
# roots.append("D:/Documents/AirSim/Scene1_Data/evening/")
# roots.append("D:/Documents/AirSim/Scene1_Data/noontime/")
# roots.append("E:/dataset/Scene2_Data/daytime/")
# roots.append("E:/dataset/Scene2_Data/noontime/")
# roots.append(r"d:\Documents\AirSim\BigNew\scene1/")
...
for root in roots:
    ...
    for dirname in dirnames:
        ...
        for i in range(len(namelist)):
            ...
            pts2 = np.float32([[0, 0], [1599, 0], [1599, 499], [0, 499]])
            M = cv2.getPerspectiveTransform(pts1, pts2)
            perspective_roi = cv2.warpPerspective(roi, M, (1600, 500), flags = cv2.INTER_NEAREST)

            semantic_map = np.ones([500, 1600]) * 65
            for (piex, index) in province_table.items():
                ...
                area = len(cols)
                result.append({'char':index, 'x1':left, 'y1':up, 'x2':right, 'y2':down, 'delete':0, 'cmean':cmean, 'area':area})
            for (piex, index) in char_table.items():
                [rows, cols] = np.where(perspective_roi[:, :, 2] == piex)
                if len(rows) < 5:
                    continue
                #print(piex)
                tmp_map = np.zeros([500, 1600])
            ...
            for i in range(len(overlap_group)):
                ...
                if flag_merge == 0:
                    ...
                    cmean /= area
                    result_sorted.append({'char':char, 'x1':left, 'y1':up, 'x2':right, 'y2':down, 'delete':0, 'cmean':cmean, 'area':area})
            ...
            LP_dict['distance'] = int(int(image_name[:-4])/50)
            LP_dict['angle'] = int(int(image_name[:-4])/50)
            ...
            if not os.path.exists(realdir+"/annotations/"):
                os.makedirs(realdir+"/annotations/")
                print(realdir+"/annotations/ is maked")
                
            with open(realdir + '/annotations/%s.json' % image_name[:-4], 'w') as f:
                f.write(json.dumps(LP_dict, sort_keys=True, indent=4, separators=(',', ':'), cls=UserEncoder))

            pbar.update(int(int(image_name[:-4])*100/len(namelist)))
        pbar.finish()
